...
<template>
    <div class="powerstatus-container">
        <div class="powerstatus-inner-container">
            <div class="powerstatus" :active="is_connected" :class="{ rpi_connected: rpi_connected }">
                ...
                <p>{{ is_connected ? "Linia online" : "Linia offline" }}</p>
            </div>
            <div v-if="!rpi_connected" style="color: orange; font-weight: 600; margin-top: 5px;">
                Raspberry Pi offline!
            </div>
        </div>
        <img class="logs-button" src="../assets/logs.png" title="Pokaż logi systemowe" 
...
.powerstatus {
    ...
    width: max-content;
    .lightning {
        filter: invert(85%) sepia(8%) saturate(3161%) hue-rotate(93deg) brightness(92%) contrast(78%);
        height: 26px;
    }
    p {
        font-size: 14px;
